...
public class EditBudgetCommandParser implements Parser<EditBudgetCommand> {
    ...
    public EditBudgetCommand parse(String args) throws ParseException {
        requireNonNull(args);
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(args, PREFIX_CATEGORY, PREFIX_AMOUNT,
        ...
        if (!arePrefixesPresent(argMultimap, PREFIX_AMOUNT) && !arePrefixesPresent(argMultimap, PREFIX_STARTDATE)
        && !arePrefixesPresent(argMultimap, PREFIX_ENDDATE) && !arePrefixesPresent(argMultimap, PREFIX_REMARKS)) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, EditBudgetCommand.MESSAGE_USAGE));
        }
